...
void PNG::_blank()
{
	for (size_t y = 0; y < _height; y++)
	{
		for (size_t x = 0; x < _width; x++)
		{
			RGBAPixel & curr = _pixel(x,y);//? why curr
			curr.red = 255;
			curr.green = 255;
			curr.blue = 255;
			curr.alpha = 255;
...
void PNG::_init()
{
	if (_pixels != NULL)//? before initializing, it should point to null
		_clear();
	_width = 1;
	_height = 1;
...
bool PNG::_pixels_same( const RGBAPixel & first, const RGBAPixel & second ) const {
    return first.red == second.red && first.green == second.green && first.blue == second.blue && first.alpha == second.alpha;//? why directly use this pointer
}
